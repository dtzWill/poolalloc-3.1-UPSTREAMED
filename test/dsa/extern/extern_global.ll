...
;RUN: dsaopt %s -dsa-steens -analyze -check-same-node=normalGlobal:0,globalIntPtr
;RUN: dsaopt %s -dsa-steens -analyze -check-same-node=internalGlobal:0,normalGlobal
;RUN: dsaopt %s -dsa-steens -analyze -check-same-node=globalStructWithPointers:8,globalStructWithPointers:8:8
